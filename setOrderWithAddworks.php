...
$additional_works = json_decode($_GET['addworks'], true);
...
else 
{
    ...
    if(count($additional_works) > 0)
    {
        ...
        $sql5 = "SELECT MAX(Order_Id) FROM Orders";
        
        $new_order_id = $db->Select($sql5)[0]['MAX(Order_Id)'];

        for($i=0; $i<count($additional_works); $i++) {
            $sql5 = "INSERT INTO `Additional_Works_In_Order` (`Work_Id`, `Order_Id`, `Type_Of_Work_Amount`)
            VALUES (".$additional_works[$i]['Work_Id'].", ".$new_order_id.", ".$additional_works[$i]['Type_Of_Work_Amount'].")";
            $insert_result = $db->Execute($sql5);
        }
